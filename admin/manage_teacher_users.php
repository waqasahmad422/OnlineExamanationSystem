...
$page_title = 'Manage Teacher Users';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        redirect('./manage_teacher_users.php');
        exit;
    }
    ...
    if (isset($_POST['create_user'])) {
        ...
        if ($result['success']) {
            ...
            set_message('success', 'Teacher user "' . htmlspecialchars($username) . '" created successfully!');
            redirect('./manage_teacher_users.php');
            exit;
        } else {
    ...
    if (isset($_POST['delete_user'])) {
        ...
        set_message('success', 'Teacher deleted');
        redirect('./manage_teacher_users.php');
        exit;
    }
    ...
    if (isset($_POST['toggle_status'])) {
        ...
        mysqli_query($conn, "UPDATE users SET is_active = $new_status WHERE id = $user_id AND role = 'teacher'");
        set_message('success', 'Teacher status updated');
        redirect('./manage_teacher_users.php');
        exit;
    }
}
...
// Load teacher users with department info
...
<div class="main-content">
    <div class="top-navbar d-flex justify-content-between align-items-center mb-3">
        <h4 class="mb-0"><i class="fas fa-chalkboard-teacher"></i> Manage Teacher Users</h4>
        <div class="d-flex align-items-center gap-2">
            <input id="tableSearch" class="form-control form-control-sm" placeholder="Search teachers..."
    ...
    <div class="m-4">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">Filter by Department</div>
                <div class="card-body">
                    <div id="deptFilter" class="btn-group flex-wrap" role="tablist">
                        ...
                        while ($d = mysqli_fetch_assoc($departments)):
                            ...
                            <button class="btn btn-outline-primary btn-sm me-1 mb-1"
                                data-dept="<?= htmlspecialchars($d['id']); ?>">
    ...
    <div class="content-area">
        <div class="card">
            ...
            <div class="card-body">
                    ...
                    <table class="table table-striped table-sm" id="usersTable">
                        ...
                        <tbody id="userTableBody">
                            ...
                            while ($user = mysqli_fetch_assoc($users)):
                                ...
                                ?>
                                <tr
                                    data-department="<?= $user['department_id'] ? htmlspecialchars($user['department_id']) : ''; ?>">
                                    ...
                                    <td>
                                        ...
                                        <form method="POST" style="display:inline;">
                                            ...
                                            <input type="hidden" name="is_active"
                                                value="<?= htmlspecialchars($user['is_active']); ?>">
                                        ...
                                        <!-- Delete icon -->
                                        <button class="btn btn-sm" type="button"
                                            onclick="openDeleteModal(<?= htmlspecialchars($user['id']); ?>)" title="Delete"
                            ...
                            <?php if (!$hasTeachers): ?>
                                <tr>
                                    <td colspan="7" class="text-center text-muted">No teacher users found. Click "Add
                                        Teacher" to create one.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                ...
                <div id="noResults" class="text-center text-muted" style="display:none;">No teacher users found.</div>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="modal-content">
            <form method="POST" id="createForm" onsubmit="return validateCreateForm()">
                ...
                <div class="modal-header">
                    <h5 class="modal-title">Create New Teacher</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-2">
                        ...
                        <input name="mobile_number" id="create_mobile_number" type="tel" class="form-control"
                    ...
                    <div class="mb-2">
                        ...
                        <select name="department_id" class="form-select">
                            ...
                            <?php
                            // Reset departments pointer for dropdown
                            mysqli_data_seek($departments, 0);
                            while ($d = mysqli_fetch_assoc($departments)):
                                ?>
                                <option value="<?= htmlspecialchars($d['id']); ?>"><?= htmlspecialchars($d['name']); ?>
                                </option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-primary" name="create_user" type="submit">Create Teacher</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="modal-content">
            <form method="POST" id="editForm" onsubmit="return validateEditForm()">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-2">
                        <label class="form-label">Mobile Number</label>
                        <input name="mobile_number" id="edit_mobile_number" type="tel" class="form-control"
                            placeholder="Enter mobile number (e.g., +92 300 1234567)"
                ...
                <div class="modal-footer">
                    <button class="btn btn-secondary" data-bs-dismiss="modal" type="button">Cancel</button>
                    <button class="btn btn-primary" name="edit_user" type="submit">Update Teacher</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST">
                ...
                <div class="modal-body">
                    <p class="mb-0">Are you sure you want to permanently delete this teacher? This cannot be undone.</p>
                </div>
                <div class="modal-footer">
                    <button class="btn btn-secondary" data-bs-dismiss="modal" type="button">Cancel</button>
