...
// Fetch teacher info
$teacher_query = mysqli_prepare($conn, "
    SELECT u.*, d.name AS department_name
    FROM users u
    LEFT JOIN departments d ON u.department_id = d.id
    WHERE u.id = ? AND u.role = 'teacher'
");
mysqli_stmt_bind_param($teacher_query, "i", $teacher_id);
mysqli_stmt_execute($teacher_query);
$result = mysqli_stmt_get_result($teacher_query);
$teacher = mysqli_fetch_assoc($result);

if (!$teacher) {
    set_message('danger', 'Teacher profile not found.');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    mysqli_stmt_bind_param($check_user, "si", $username, $teacher_id);
    ...
    mysqli_stmt_bind_param($check_email, "si", $email, $teacher_id);
    ...
    $update = mysqli_prepare($conn, "
        ...
        WHERE id = ?
    ");
    mysqli_stmt_bind_param($update, "sssi", $full_name, $username, $email, $teacher_id);
...
/* ===========================
    CHANGE PASSWORD (NO CURRENT REQUIRED)
   =========================== */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
    ...
    ");
    mysqli_stmt_bind_param($pwd, "si", $hashed, $teacher_id);
...
<div class="main-content">
    <div class="top-navbar d-flex justify-content-between align-items-center mb-4">
        <h4 class="mb-0"><i class="fas fa-user"></i> Teacher Profile</h4>
            ...
            <div class="col-md-4">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body text-center">
                        ...
                        <h4 class="mb-1"><?= htmlspecialchars($teacher['full_name']); ?></h4>
                        <p class="text-muted">@<?= htmlspecialchars($teacher['username']); ?></p>
                        <span class="badge bg-success">Active</span>

                        <hr>

                        <p class="mb-1"><strong>Department:</strong> <?= $teacher['department_name'] ?: 'N/A'; ?></p>
                        <p class="mb-1"><strong>Joined:</strong> <?= date('M Y', strtotime($teacher['created_at'])); ?></p>
                    </div>
                </div>
            </div>
